...
func layer0ReducerSpecs() []layer0ReducerSpec {
	return []layer0ReducerSpec{
		...
		diagnosticReducer("tier1_eslint_stylish", "lint", []string{"rule id", "severity", "path", "line", "column", "message", "summary count"}, TryCompactEslintStylish),
		...
		argvReducer("known_cli_json_exact", "json", Layer0ReducerSafetyExact, []string{"all CLI JSON bytes", "valid JSON fields", "valid JSON scalar values", "array order"}, TryCompactKnownCLIJSONExact),
