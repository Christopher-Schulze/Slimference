...
### Phase R - Effectiveness + Stealth Boost (planned 2026-05-01)

The previous phases built the plumbing (transparent mode, redaction, capability matrix). Phase R is where Slimference becomes objectively better than direct-to-OpenAI on every measurable axis: undetectable on the wire, smarter about programming-language tool output, AST-aware about code reads, image-format-optimal, conversation-state-diffing-aware, output-token-reducing. All eight tasks are lossless by design - no information the agent depends on is lost; only redundancy and ceremony are removed. Sequencing left-to-right: T123 (TLS stealth) ships first because it changes the wire-level signature; T124-T128 land in parallel pairs after; T129 + T130 last because they are policy/config flips that piggyback on the rest.

- [ ] T123 - TLS-fingerprint mimicry (uTLS-based browser/CLI profile matching) so Slimference's outbound TLS Client Hello becomes byte-for-byte identical to the upstream tool's native stack (Codex Desktop = Electron/Chromium, Codex CLI = Node, Claude Code = Anthropic SDK). Closes the JA3-fingerprint gap that is theoretically detectable today. Detail: `docs/todo/t123-tls-fingerprint-mimicry.md`
- [ ] T124 - Layer 0 per-language structured parsers for programming-language tool output (Go gopls/go vet, Rust cargo build/test/clippy, Python mypy/pylint/ruff/pytest, JS/TS tsc/eslint/jest/vitest, C/C++ gcc/clang/cmake/make, Java javac/mvn/gradle, Kotlin/Swift/Dart/Ruby/PHP/Elixir/Haskell/Scala). Replaces the generic build/test heuristic with per-tool semantic compaction. Detail: `docs/todo/t124-layer0-language-parsers.md`
- [ ] T125 - AST-based code compaction for file-read tool results: when an agent reads a 2000-line code file, the tool result becomes (package decl + imports + symbol table + signatures + the touched function bodies); subsequent reads of the same file send only the diff. Lossless: agent can re-grep / re-read for any body it needs. Detail: `docs/todo/t125-ast-code-compaction.md`
- [ ] T126 - Cross-tool result deduplication: state machine that tracks which file paths / resource lists / status digests the agent has already seen this turn, and elides redundant repeats in subsequent tool results. (`git status` then `git diff` no longer ships the modified-files list twice.) Detail: `docs/todo/t126-cross-tool-dedup.md`
- [ ] T127 - Image format optimisation: when an agent receives an image (typically Codex Desktop screenshots), Slimference re-encodes it to the smallest format that preserves identical visual output (PNG/lossless-WebP for screenshots with text, JPEG/WebP-VQ for photos at the upstream-specified quality, GIF -> APNG/WebP for animation). 60-90% Image-token saving with zero visual loss. Detail: `docs/todo/t127-image-compression.md`
- [ ] T128 - Conversational state diffing: aggressive prompt-cache breakpoint placement so Anthropic's prompt cache fires on every system+conversation prefix (not just system); for OpenAI (no native prompt-caching API) maintain a Slimference-side hash log of last-sent prefix per session and chunk requests so the provider sees stable repeated prefixes. Saving on long-conversation turns 50-80%. Detail: `docs/todo/t128-conversational-state-diffing.md`
- [ ] T129 - Layer 2 default-ON re-flip: reverse T121's default-off after T127 + T128 + the T109 redactor have proven the data-policy stance is honest. MiniMax stays as the configured summarizer (operator already has the subscription); doctor's per-provider trust-label warning stays visible. Detail: `docs/todo/t129-layer2-default-on.md`
- [ ] T130 - Layer 4 output-token compression: inject per-provider system-prompt directives that minimise the model's output ceremony (no preambles, diff-only for code edits, no quoting back received content), measured against an output-token-saving counter. Output tokens are 3-5x more expensive than input; this is the largest unrealised cost lever. Detail: `docs/todo/t130-output-token-compression.md`
