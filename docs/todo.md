...
- [~] T138 - Session/turn ownership spine for AST and cross-tool state is in progress: the bounded `internal/sessions.TurnStateStore` core is landed with session/turn/tool/file/git/prompt/quality observations and 100% package coverage. Hot-path hook persistence, T125 FileReadContext plumbing, T126 per-turn filter integration, and body-on-demand retrieval remain open. Detail: `docs/todo/t138-session-turn-ownership-spine.md`
- [ ] T139 - TLS fingerprint provider-edge proof and profile maintenance: extend T123 from local uTLS proof to reflected JA3/JA4/ALPN/HTTP2 evidence, profile freshness automation, and honest provider-invisibility reporting without "undetectable" claims. Detail: `docs/todo/t139-tls-provider-edge-proof.md`
- [ ] T141 - Output-token reduction auto-tuning and failure guard: take T130 from static prompt injection to measured per-provider/per-model output discipline with auto-soften/disable, failure correlation, and aggressive-but-safe Codex-specific rules. Detail: `docs/todo/t141-output-token-auto-tuning.md`
- [ ] T140 - Codex CLI/App live E2E certification and real corpus: certify the full product path on the user's machine with Codex CLI, Codex App, Browser-Use, WebSocket mode, voice/WebRTC bypass, disable/uninstall, and scrubbed corpus savings proof. Detail: `docs/todo/t140-codex-live-e2e-certification.md`
