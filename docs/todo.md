...
- [~] T138 - Session/turn ownership spine for AST and cross-tool state is in progress: the bounded `internal/sessions.TurnStateStore` core is landed, Codex hook processes share file-backed turn-state under `~/.slimference/turn-state/`, T125 FileReadContext keeps recently-edited reads literal, T125 has archive-backed `expand-body` recovery, T126 has safe Codex PostToolUse same-turn git path-list elision, TUI Debug shows the latest hook turn-state, and persistent stores now share `sessions.SafeSessionID` for storage keys. Remaining: turn-level convergence/live proof across read cache, repetition, tool archive, quality, and proxy. Detail: `docs/todo/t138-session-turn-ownership-spine.md`
...
- [ ] T140 - Codex CLI/App live E2E certification and real corpus: CLI-only split proof is certified for the current Codex CLI without arming macOS System-HTTPS-Proxy (`proxy env codex --proxied`, per-process `slimference-codex` custom provider, `supports_websockets=false`, App remains direct). Fresh 2026-05-14 exact-reply smoke returned `SLIMFERENCE_T140_FRESH_OK` through `/backend-api/codex/responses` with provider-reported input/cache/output accounting and all macOS Network services still `off`. Default WebSocket tunnel works byte-for-byte; the old `direct_codex_websocket_policy = "force_https_fallback"` path remains only as fallback proof mode. Live CLI smoke, CLI tool-loop, and larger read-only CLI repo-audit probe passed through the zstd HTTP pipeline with provider-reported input/cache/output accounting. `gain --proxy` and `savings` now report provider-only decision-log flight accounting for real proxied Codex traffic without local hook/readhook inflation. Remaining proof: Codex App/system-proxy E2E, Browser-Use passthrough, voice/WebRTC bypass, disable/uninstall, scrubbed corpus savings, and WebSocket frame-shape/compression feasibility. Detail: `docs/todo/t140-codex-live-e2e-certification.md`
...
- [ ] T143 - Layer 1 semantic deterministic compaction frontier: T143a reversible path dictionary, T143b Markdown/SQL/GraphQL/HCL/Dockerfile/Makefile structure extraction, and T143c local-tokenizer structure budget gate landed. Remaining: provider-context-specific budgets, deeper multi-language symbol slicing, stacktrace/test-failure semantics, config/schema deltas, and quality/live-corpus gates. Detail: `docs/todo/t143-l1-semantic-deterministic-frontier.md`
