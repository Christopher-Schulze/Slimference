...
- [ ] Extend `readcache.Request` + `IsFullFileRead` handling to support ranged reads;
...
- [ ] Add cross-turn search-output reference/delta for repeated identical queries
      (reuse the content-archive + session-scoped store from the T248 non-file dedup).
- [ ] Fixtures proving ranged-read and search re-read mutation; verify per-route
