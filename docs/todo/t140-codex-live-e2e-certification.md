...
- 2026-05-14 larger CLI-only read-only probe completed through the same `slimference-codex` per-process provider path:
  ...
  - `slimference gain --proxy` now aggregates decision-log flight records for real proxied requests and reports provider input/cache/output tokens, input savings estimate, output-reduce overhead, and cache-read billing-equivalent credit. On the local 2026-05-14 decision log it reported `1518` proxy requests, `921575` provider input tokens, `754688` provider cached tokens, `5030` provider output tokens, `250021` billable input savings estimate, `1053` output-reduce input overhead tokens, and `929240` net billing-equivalent estimate.
  - `gain --cache` and `gain --output` can still be zero when only decision-log flight recording is configured; for Codex CLI/App proxy proof, `gain --proxy` is the evidence command.
- 2026-05-14 post-fix installed-binary smoke:
  - Rebuilt `/Users/christopher/.local/bin/slimference`, restarted the local daemon, and verified it running on PID `28648`, port `8990`.
  - Ran a new Codex CLI read-only smoke through the per-process `slimference-codex` provider after the daemon restart. Codex returned `SLIMFERENCE_DAEMON_RESTART_OK`.
  - Flight evidence for request `282aff8f6a03cca5` showed `route_mode=upstream`, `provider=codex_chatgpt`, `path=/backend-api/codex/responses`, `confidence=provider_reported`, `provider_input_tokens=32467`, `provider_cached_tokens=7552`, `provider_output_tokens=81`, and `output_reduce.reason=below_min_tokens`.
