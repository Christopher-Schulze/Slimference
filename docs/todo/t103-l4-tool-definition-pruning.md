...
- [ ] Tool definitions for tools idle beyond threshold are removed from the request.
- [ ] Reattach path reattaches transparently on first use after pruning.
- [ ] No regression on tools that are always used.
- [ ] Counters surface in `/admin/status.toolprune`.
- [ ] T77 quality signals show no spike in re-read rate after L4 enables.
