...
- `go run ./scripts/ci` passes all 8 gates under the current aggregate
  coverage policy.
- Coverage wording is intentionally "total coverage": `scripts/ci`
  enforces the aggregate `go run ./scripts/coverage -min=99.5` gate, not
  per-package output for every package line.
