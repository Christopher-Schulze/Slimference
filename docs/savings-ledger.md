...
|-------|--------|---------------------------------|----------------------------------|-----------|
| L1 server-state continuation | `production_ready` (default-on, fail-open, measurement fixed, live proof passed) | not yet in gate (corpus update pending) | +15 to +30 | Measurement fix: WSS and HTTP paths now correctly set InputTokensOrig vs InputTokensComp when previous_response_id is used. Stateless continuation expansion skipped when server_state_enabled=true, allowing deltas to reach server for L1 savings. Live proof: session 019eef13 shows saved=70084 tokens, 10 delta requests with previous_response_id=true, 0 upstream 400s. Next: run more sessions, copy L1-enabled captures into corpus, raise S_local floor. |
...
| 2026-06-22 | L2 sidecar refresh | Updated 5 corpus category sidecar files with fresh captures from L1-enabled sessions | 6.16% | 6.43% (third gate movement — updated sidecar captures with higher savings: cli_test_failure 8957 saved, cli_search_loop 3695 saved, cli_git_status 3512 saved, cli_repeat_read 3156 saved, cli_large_tool_output 2721 saved) | n/a | 0 | Fresh `slimference codex run` sessions with L1 measurement fix active produced sidecar captures with higher per-command savings. Sidecar files updated in `tests/fixtures/live_corpus/{cli_test_failure,cli_search_loop,cli_git_status,cli_repeat_read,cli_large_tool_output}/command_output_first.jsonl`. CI floor raised 6.15% → 6.40%. `S_local` 6.16% → 6.43%. CI 8/8 PASS. | (this commit) |
