...
3. Built-in reducers tried in priority order from
   ...
   truncated by default. `gh`/`glab` API calls and `jq` JSON output follow
   exact-only paths before generic JSON minify. Known CLI JSON modes
   (`kubectl`/`oc -o json`, `cargo metadata`, and `terraform`/`tofu show|output -json`)
   also use an exact fallback after their structured parsers decline, so large
   API/VCS/inspected JSON payloads cannot be schema-summarized or TOML-truncated
   by default. Long `terraform state list` and plain human-readable
...
defaults off because top-level `tools` are model-facing capability context in
...
guarded path for scoped proof runs, but it is not a product activation latch.
Provider-cache protection is exact-scope inside that lab path: if a comparable
usage frame shows a cached-token share drop after a prefix-elided turn, the
cache-bust guard demotes stateful tool-prefix elision for only that
request-shape + `prompt_cache_key` scope. Decision debug facts report
...
`request_user_input`. Live scoped WSS proof showed that removing that plan-only
tool from a root request could still be followed by a client-side
...
The WSS full-history default slice can be disabled for a hot rollback with
`SLIMFERENCE_WSS_FULL_HISTORY_TOOL_PRUNE=0`.
...
when broad tool-prune is enabled, or on previous-response-id `full_history`
frames through the default-safe WSS slice, with a known Codex tool schema.
Unknown-session tools remain attached by the fail-open tracker, so no-user root
frames prune only after prior session evidence proves a tool idle and the broad
operator flag is on. Previous-response-id delta turns still full-pass general
...
session bucket. The cooldown keeps the full schema for one follow-up prune
decision, warms the just-pruned definitions as active, and then restores normal
pruning instead of disabling unrelated tool-schema savings for the session.
`slimference
