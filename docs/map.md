...
- `internal/contextledger/message_apply.go`: exact full-history message/block apply primitive with archive byte-match, exact archive-to-message target derivation, selected-target token accounting, and full-pass gates for unsafe targets
- `internal/abharness/abharness.go`: direct-vs-compressed comprehension replay harness; expands OCRL archive lists and deleted-block coverage before classifying context as lost
