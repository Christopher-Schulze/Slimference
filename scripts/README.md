...
go run ./scripts/utils wss-proof-pack ~/.slimference/debug/decisions.jsonl --since-file=/tmp/slimference-desktop-proof-since.txt --json # content-free T417/T420/T408 proof-window pack: combines class distribution, strict local-gap instrumentation, backend-reference inventory, and the matching `slimference debug wss-sockets` command; fails closed on stale/missing current WSS ownership facts or upstream/400 errors so broad savings work cannot promote from dirty historical rows
