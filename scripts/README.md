...
go run ./scripts/utils wss-t354-shape-proof ~/.slimference/captures/codex-wss-frames.jsonl --t420-handoff-json=/tmp/wss-sockets.json --json # content-free T417 Class-B/server-state candidate proof; optional T420 handoff JSON ingests reconnect full-history rows from `slimference debug wss-sockets --json` as first-class reroute/transport-fix candidate inputs
