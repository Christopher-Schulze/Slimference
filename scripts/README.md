...
go run ./scripts/utils wss-proof-pack ~/.slimference/debug/decisions.jsonl --since-file=/tmp/slimference-desktop-proof-since.txt --sockets-json=/tmp/wss-sockets.json --json # content-free T417/T420/T408 proof-window pack: combines class distribution, strict local-gap instrumentation, backend-reference inventory, optional `slimference debug wss-sockets --json` handoff ingestion, and the matching socket command; fails closed on stale/missing current WSS ownership facts, upstream/400 errors, or reconnect full-history mass without handoff rows so broad savings work cannot promote from dirty historical rows
