...
go run ./scripts/utils wss-reference-inventory ~/.slimference/captures --json       # T408 content-free backend-reference discovery: counts reference-like JSON fields/raw mentions and local archive URI markers without printing payload values; no arbitrary backend reference => server-state mirror product mutation stays off
