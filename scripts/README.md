...
go run ./scripts/utils wss-proof-matrix captures/search-proof.jsonl --require-live-token-delta --required-workload=search_loop --min-captures=2 --min-cli=1 --min-desktop=1 --min-positive=2 --expected-reducer codex_exec_envelope --search-cap-candidate=30:15 --search-cap-candidate=25:15 --json # focused search-loop mechanism gate with optional T359 search-cap promotion proof, not a release substitute; coverage/savings aggregates count only row-gate-passed captures; search-cap defaults fail closed at 40% retained matches, >=2 resolved search outputs, +1 extra reducer token, and delta tool-output mutation proof
