...
go run ./scripts/utils wss-local-gap ~/.slimference/debug/decisions.jsonl --min-local-ratio=0.48 --json # strict AGENTS.md 3.2 S_local gap ledger; provider-cache is separate; actionable_potential separates proof-latch candidates, unsafe guards, and no-evidence instrumentation gaps
