...
go run ./scripts/utils wss-ab-replay captures/codex-wss-frames.jsonl --fail-on-lost # product-default byte-equal WSS replay gate
go run ./scripts/utils wss-ab-replay captures/codex-wss-frames.jsonl --json          # machine-readable A/B report
go run ./scripts/utils wss-ab-replay captures/codex-wss-frames.jsonl --tool-output-mutation --fail-on-lost --json # lab/proof WSS reducer replay; known output-reduce instruction suffixes are audited as expected extras, unknown instruction rewrites still fail lost gate
