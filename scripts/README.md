...
go run ./scripts/utils wss-ab-replay captures/codex-wss-frames.jsonl --stateful-prefix-elision-proof --fail-on-lost --json # proof-only WSS prefix-elision replay; elides repeated top-level tools/instructions only after the same canonical prefix was already seen in the same prompt_cache_key scope
