...
go run ./scripts/utils wss-reference-inventory ~/.slimference/captures --json       # T408 content-free backend-reference discovery: counts reference-like JSON fields/raw mentions and local archive URI markers without printing payload values; emits Lane-3 accepted-contract schema, per-field verdicts, and re-probe triggers; no accepted backend reference => server-state mirror product mutation stays off
go run ./scripts/utils wss-first-read-inventory ~/.slimference/debug/decisions.jsonl --json # T413 content-free first-read/scan-read dependency audit: counts read-like/source-tool candidate mass, existing read_delta seeding/savings, re-read/context-risk signals, and file/range/hash/edit-lineage dependency trace coverage; missing trace keeps first_read_elision shadow-only
go run ./scripts/utils wss-post-edit-inventory ~/.slimference/debug/decisions.jsonl --json # T414/T415 content-free post-edit/patch-context audit: counts post-edit read surface, exact file-state facts, repeated unchanged post-edit candidates, patch/diff hash telemetry, and conflict/failure blockers; missing exact-state keeps predictive_post_edit and patch_context_dedup shadow-only
go run ./scripts/utils wss-local-gap ~/.slimference/debug/decisions.jsonl --min-local-ratio=0.48 --json # strict AGENTS.md 3.2 S_local gap ledger; provider-cache is separate; reports all-request S_local, positive-savings-request ratio, and policy_savings_ceiling after protected capability-prefix/context and known-non-target mass; actionable_potential separates proof-latch candidates, unsafe guards, no-evidence gaps, and protected capability-prefix byte mass; unattributed_gap rows include top_sessions so residual mass becomes rankable by exact session/shape/mechanism/reason instead of staying abstract
go run ./scripts/utils wss-class-distribution ~/.slimference/captures --json # content-free billing-class decomposition (roadmap section 1): splits WSS input mass into protected prefix (Class C), reducible tool-output (the Layer-0 target, optimistic ceiling), and other context (messages + Class-D reasoning) per request class (delta/full_history/root); on fresh debug-fact captures, root_context_ledger further splits other-context bytes into messages, function calls, function-call outputs, reasoning, raw-other, and non-input envelope for T408/T417 ownership ranking; reducible_ceiling_ratio, headroom_present, gap_inventory_recommended, and next_action answer whether the 48% S_local target is reachable in this corpus/session class or blocked by a corpus ceiling; add --since-file=/tmp/slimference-desktop-proof-since.txt for owner Desktop proof windows and --require-headroom when this is used as the guard-work gate; provider-cache is separate
go run ./scripts/utils wss-proof-pack ~/.slimference/debug/decisions.jsonl --since-file=/tmp/slimference-desktop-proof-since.txt --sockets-json=/tmp/wss-sockets.json --audit-json=/tmp/wss-audit.json --json # content-free T417/T420/T408 proof-window pack: combines class distribution, strict local-gap instrumentation, backend-reference inventory, optional `slimference debug wss-sockets --json` handoff ingestion, optional `wss-audit --json` shadow-mirror/headroom ingestion, and the matching commands; fails closed on stale/missing current WSS ownership facts, upstream/400 errors, or reconnect full-history mass without handoff rows so broad savings work cannot promote from dirty historical rows or hide large T408/T418 headroom as "no mass"
go run ./scripts/utils wss-t354-shape-proof ~/.slimference/captures/codex-wss-frames.jsonl --t420-handoff-json=/tmp/wss-sockets.json --t408-open-slice-json=/tmp/wss-audit.json --require-recovery-contract --json # content-free T417 Class-B/server-state candidate proof; optional T420 handoff JSON ingests reconnect full-history rows from `slimference debug wss-sockets --json`; optional T408 open-slice JSON ingests exact non-demoted `wss-audit` open-scope candidates as first-class T417 activation inputs; --require-recovery-contract attaches the T419 gate so bytes-omitting Class-B promotion cannot pass with missing archive/reinject/fail-open/accounting contracts
...
go run ./scripts/utils search-cap-proof --frames ~/.slimference/captures/codex-wss-frames.jsonl --candidate=30:15 --candidate=25:15 --min-candidate-retained-pct=40 --min-search-outputs=2 --min-extra-reducer-tokens=1 --json # combined T359 promotion gate: product baseline, proof-latched default/candidate replays, profile breadth, retention, WSS replay lost=0/upstream-error-free, and positive extra savings over the current product path
go run ./scripts/utils codex-capture-run --binary ~/.local/bin/slimference --port=8991 --capture ~/.slimference/captures/repeat.jsonl --decisions-log ~/.slimference/captures/repeat.decisions.jsonl --matrix-row /tmp/proof-matrix.jsonl --id cli-repeat --workload-class repeat_full_read --expected-reducer read_delta --min-function-calls=2 --min-function-call-outputs=2 --codex-timeout=180s --exit-marker CAPTURE_DONE --exit-marker-count=2 --quiet-codex-output -- exec "Run exactly two shell tool calls and do not modify files. First tool call cmd exactly: cat AGENTS.md Second tool call cmd exactly: cat AGENTS.md Then final message exactly CAPTURE_DONE" # records live billable input-token delta plus replay lost=0 bytes on a managed proof daemon; --decisions-log records content-free request decisions and writes decisions_path into the matrix row for wss-audit; --port can use a separate local port with isolated daemon PID/lock state so an existing normal daemon on 8990 is not stopped; --expected-reducer and tool-call minima are enforced against live evidence before PASS, but the evidence row is still appended for failed focused runs; marker exit watches captured server output and function_call_output frames while ignoring raw PTY prompt echo
go run ./scripts/utils codex-capture-run --binary ~/.local/bin/slimference --port=8991 --transport=wss --search-cap-proof-lab=20:10 --search-cap-min-retained-pct=40 --workload-class search_loop --expected-reducer captured_output --matrix-row /tmp/search-cap-proof.jsonl --exit-marker SEARCH_CAP_DONE --quiet-codex-output -- exec "<scoped search-loop prompt with at least one clean following tool turn>" # proof-only bootstrap for T405 live downstream-state capture; sets the narrow named-search cap only inside the managed child daemon, never via persistent config or shared Codex route
...
go run ./scripts/utils wss-ab-replay captures/codex-wss-frames.jsonl --stateful-prefix-elision-proof --fail-on-lost --json # proof-only WSS tool-prefix-elision replay; elides repeated top-level tool schemas only after the same canonical prefix was already seen in the same prompt_cache_key scope; instructions stay byte-equal
...
go run ./scripts/utils wss-savings-baseline ~/.slimference/captures --json # content-free baseline over local WSS frame captures: product default, proof-latched search-cap, broad non-delta lab, and guarded/unsafe savings gaps
...
go run ./scripts/utils command-output-control-probe --json --shim-command=git -- /bin/sh -c 'git --version >/dev/null; "$SHELL" -c true' # T418 process-local control-point probe: reports whether SHELL, BASH_ENV, and PATH shims fire for the child process without touching shell rc files, Codex config, global proxy, hosts, pfctl, browser ChatGPT, or ordinary ChatGPT.app; wrap `slimference codex run ...` as the child for the scoped Codex live proof
go run ./scripts/utils recovery-contract-matrix --json --fail-on-product-gaps # T419 content-free recovery contract gate: inventories Layer-0 parser reducers, T418 archive-backed command-output-first rows, expand/reinject recovery, T417/T408 candidate blockers, and fails only if default-eligible product rows lack exact recovery/fail-open/accounting contracts
