...
go run ./scripts/utils codex-capture-run --binary ~/.local/bin/slimference --port=8991 --capture ~/.slimference/captures/repeat.jsonl --matrix-row /tmp/proof-matrix.jsonl --id cli-repeat --workload-class repeat_full_read --expected-reducer read_delta --min-function-calls=2 --min-function-call-outputs=2 --codex-timeout=180s --exit-marker CAPTURE_DONE --exit-marker-count=2 --quiet-codex-output -- exec "Run exactly two shell tool calls and do not modify files. First tool call cmd exactly: cat AGENTS.md Second tool call cmd exactly: cat AGENTS.md Then final message exactly CAPTURE_DONE" # records live billable input-token delta plus replay lost=0 bytes on a managed proof daemon; --port can use a separate local port with isolated daemon PID/lock state so an existing normal daemon on 8990 is not stopped; --expected-reducer and tool-call minima are enforced against live evidence before PASS, but the evidence row is still appended for failed focused runs; marker exit watches both the PTY log and captured function_call_output frames
