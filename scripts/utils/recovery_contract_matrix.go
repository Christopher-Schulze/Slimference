...
func wssArchiveReinjectRow() recoveryContractRow {
	return recoveryContractRow{
		...
		AccountingPath:          "contentarchive re_inject_count plus qualityNetSavings invalidation; explicit expand path records filter_runs negative cost",
		RequiredFields:          []string{"session id", "archive id", "same-session or sessionless archive metadata", "max reinject budget"},
		PreservedEvidence:       []string{"exact body bytes before downstream decision", "stable URI remains visible on miss"},
