...
func TestHandleReadHookCmd(t *testing.T) {
	...
	t.Run("usage_when_terminal", func(t *testing.T) {
		...
		_, _ = io.Copy(&buf, rp)
		if !exited || code != 1 || !strings.Contains(buf.String(), "usage: slimference readhook [claude|codex]") {
	...
	t.Run("blocked_read_emits_hook_json", func(t *testing.T) {
		home := t.TempDir()
		file := filepath.Join(home, "main.go")
		if err := os.WriteFile(file, []byte("package main\n"), 0o644); err != nil {
			t.Fatal(err)
